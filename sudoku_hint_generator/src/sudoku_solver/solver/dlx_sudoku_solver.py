...
class SudokuSolver(object):
    ...
    def insert_value(self, row, col, value):
        ...
        vertical_row_idx = (
            (row * self.dimension + col) * self.dimension + value + self.cell_offset
        )
    ...
    def add_row(self, row, col1, col2, col3, col4):
        ...
        row_first_node = Node(
            up=col_header_node.up, down=col_header_node, col_header=col1, row_header=row
        )
...
def solve_dlx(board):
    inboard = list((np.array(board) - 1).flatten())
    res = SudokuSolver(inboard).answer
    if res[0] == -1:
        return False
    return list((np.array(res) + 1).reshape(9, 9))
