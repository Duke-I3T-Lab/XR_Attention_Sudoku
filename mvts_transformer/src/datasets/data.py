...
class SudokuData(BaseData):
    """
    Dataset class for Sudoku dataset.
    Attributes:
        all_df: dataframe indexed by ID, with multiple rows corresponding to the same index (sample).
            Each row is a time step; Each column contains either metadata (e.g. timestamp) or a feature.
        feature_df: contains the subset of columns of `all_df` which correspond to selected features
        ...
        if self.n_proc > 1:
            ...
            with Pool(processes=_n_proc) as pool:
                all_feature_df = pd.concat(
                    pool.map(SudokuData.load_single, feature_paths)
                )
                all_label_df = pd.concat(
                    pool.map(
                        partial(SudokuData.load_single, is_label=True), label_paths
                    )
                )
        else:  # read 1 file at a time
            all_feature_df = pd.concat(
                SudokuData.load_single(path) for path in feature_paths
            )
            all_label_df = pd.concat(
                SudokuData.load_single(path, is_label=True) for path in label_paths
            )
        return all_feature_df, all_label_df

    @staticmethod
    def load_single(filepath, is_label=False):
        df = SudokuData.read_data(filepath, is_label=is_label)
        df = SudokuData.select_columns(df)
